...
const leerProductos = () => {
    return new Promise((resolve, reject) => {
        fs.readFile("productos.json", "utf8", (error, data) => {
            if(error) return reject("Ha ocurrido un error al cargar los libros.");
...
app.route("/api/productos")
    .get((req, res) =>{
        fs.readFile("productos.json", "utf8", (error, data)=> {
            if(error) return res.status(500).end({code: 500, message: "No se ha podido acceder al listado de libros"});
            let productos = JSON.parse(data);
            res.json(productos);
        })
    })
    ...
    .put((req, res) =>{
        fs.readFile("productos.json", "utf8", (error, data)=> {
            ...
            fs.writeFile("productos.json", JSON.stringify(productos, null, 4), "utf8", (error) => {
                if(error) return res.status(500).end({code: 500, message: "No se ha podido acceder al listado de libros"});
                res.json({code: 201, message:"Libro actualizado correctamente."})
            })
    ...
    .delete((req, res) =>{
        ...
        if(id){
            ...
            res.status(400).send({code: 400, message: "debe proporcionar el id del libros a eliminar."})
        }
    })
